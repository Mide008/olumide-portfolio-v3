...
if (!reduceMotion && matchMedia('(hover:hover)').matches) {
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    if (!visible) { visible = true; tag.classList.add('is-visible'); }
  });
  window.addEventListener('mouseleave', () => { visible = false; tag.classList.remove('is-visible'); });
  gsap.ticker.add(() => {
    cx += (mx - cx) * 0.22; cy += (my - cy) * 0.22;
    tag.style.transform = `translate(${cx}px, ${cy}px) translate(-50%,-50%) scale(1)`;
  });
}
...
if (matchMedia('(hover:hover)').matches && !reduceMotion) {
  document.querySelectorAll('.cap-item, .mod-grid .mod, .stat-strip .cell, .agent-row .a, .work-card').forEach((el) => {
    el.style.transformStyle = 'preserve-3d';
    const isCard = el.classList.contains('work-card');
    el.addEventListener('mousemove', (e) => {
      const r = el.getBoundingClientRect();
      const px = (e.clientX - r.left) / r.width - 0.5;
      const py = (e.clientY - r.top) / r.height - 0.5;
      gsap.to(el, { rotateY: px * 6, rotateX: py * -6, y: isCard ? -6 : 0, duration: 0.4, ease: 'power2.out', transformPerspective: 500 });
    });
    el.addEventListener('mouseleave', () => gsap.to(el, { rotateY: 0, rotateX: 0, y: 0, duration: 0.6, ease: 'power3.out' }));
  });
}
/* ---------- footer watermark reveal ---------- */
document.querySelectorAll('.footer-mark').forEach((el) => {
  gsap.fromTo(el, { opacity: 0, scale: 0.9, y: 20 }, {
    opacity: 0.05, scale: 1, y: 0, duration: 1.4, ease: 'power2.out',
    scrollTrigger: { trigger: el, start: 'top 95%' }
  });
});
